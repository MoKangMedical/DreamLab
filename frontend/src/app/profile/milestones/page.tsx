...
const MILESTONES: Milestone[] = [
  { id: 'face_shadow', name: '穿越遗忘之桥', desc: '完成第一次测评，面对自己的阴影', icon: '◇', color: '#c4554d', done: true, quote: '"不要回头，一直向前。"' },
  { id: 'know_desire', name: '汤婆婆的锅炉房', desc: '学习CBT课程，识别欲望与思维模式', icon: '◎', color: '#d4a853', done: true, quote: '"在油屋不工作的人会变成动物。"' },
  { id: 'accept_lonely', name: '无脸男的浴场', desc: '与AI陪伴对话，接纳孤独', icon: '◈', color: '#6b5b8a', done: true, quote: '"好寂寞...好寂寞啊。"' },
  { id: 'purify', name: '河神的净化', desc: '完成10次冥想，释放情绪淤积', icon: '⬡', color: '#3b8b7a', done: false, quote: '"啊...舒服多了。"' },
  ...
  return (
    <div style={{ background: '#0a0a0c', minHeight: '100vh' }}>
      <div className="max-w-4xl mx-auto px-4 md:px-6 pt-14 md:pt-20 pb-32">
        ...
        <div className="mb-12 p-6" style={{ background: '#111113', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div className="flex flex-wrap justify-between gap-4">
            {MILESTONES.map((m, i) => (
              <div key={m.id} className="flex-1 min-w-[120px] text-center">
                ...
                <div className="text-xs font-semibold mb-1" style={{ color: m.done ? '#a1a1aa' : '#52525b' }}>{m.name}</div>
                <div className="text-xs leading-relaxed" style={{ color: '#52525b', fontSize: 10 }}>{m.desc}</div>
                {m.done && (
            ...
            {ACHIEVEMENTS.map(a => (
              <div key={a.id} className="p-4 transition-all duration-300"
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <div className="text-sm font-semibold" style={{ color: a.unlocked ? '#f4f4f6' : '#71717a' }}>{a.name}</div>
                    <div className="text-xs" style={{ color: '#52525b' }}>{a.condition}</div>
                  </div>
        ...
        <div className="mb-12" style={{ background: '#111113', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold" style={{ color: '#f4f4f6' }}>成长报告</h3>
                <p className="text-xs" style={{ color: '#52525b' }}>基于你的使用数据生成</p>
              </div>
            </div>
            <span style={{ color: '#52525b', transform: showReport ? 'rotate(180deg)' : 'none', transition: 'transform 0.3s' }}>▼</span>
          </div>
          {showReport && (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={item.label} className="p-3 text-center" style={{ background: '#0a0a0c' }}>
                    ...
                    <div className="text-xs" style={{ color: '#52525b' }}>{item.sub}</div>
                  </div>
                ))}
              </div>
            </div>

...
const CATEGORIES: Record<string, { name: string; desc: string; icon: string; color: string }> = {
  anxiety: { name: '焦虑评估', desc: '了解你的紧张与不安从何而来', icon: '🌊', color: '#c47868' },
  depression: { name: '抑郁评估', desc: '正视情绪的低谷，是疗愈的第一步', icon: '🌧️', color: '#9b8ab8' },
  personality: { name: '人格探索', desc: '没有好坏，只有独一无二的你', icon: '🎭', color: '#e2b64f' },
  sleep: { name: '睡眠健康', desc: '每一个梦境，都需要安稳的港湾', icon: '🌙', color: '#7eb8da' },
  resilience: { name: '心理韧性', desc: '像竹子一样，被压弯却不断裂', icon: '🌱', color: '#8aaf9d' },
  symptom: { name: '综合筛查', desc: '360°扫描，看清心灵的全貌', icon: '📋', color: '#e2b64f' },
};
...
function AssessmentCard({ a, index }: { a: typeof ASSESSMENTS[0]; index: number }) {
  ...
  return (
    <div ref={ref} className={`transition-all duration-800 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-16'}`}
      ...
      <div className="p-8 md:p-10 mb-2" style={{ background: '#0c0c0e', border: '1px solid #1c1c1f', borderRadius: 2 }}>
        {/* 头部 */}
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            <span className="text-4xl">{a.icon}</span>
            <div>
              <h3 className="text-xl font-bold mb-1" style={{ fontFamily: "'Noto Serif SC', serif", color: '#fafafa' }}>
                {a.name}
              </h3>
          ...
          <Link href={`/assessments/${a.id}`}
            ...
            style={{
              ...
              padding: '12px 28px', background: cat.color, color: '#09090b',
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div style={{ background: '#09090b', padding: '14px 16px', borderRadius: 2 }}>

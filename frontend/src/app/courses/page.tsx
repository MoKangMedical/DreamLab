...
import { KANGBO_CATEGORIES, KANGBO_PHASES } from '@/lib/kangbo-courses';
...
export default async function CoursesPage() {
  ...
  const totalMinutes = courses.reduce((sum: number, course: any) => sum + (course.minutes || 0), 0);
          ...
          <div className="max-w-6xl mx-auto">
            <div className="max-w-3xl">
              ...
              <p className="text-base md:text-lg leading-8 max-w-2xl" style={{ color: '#a1a1aa' }}>
                65 门课程、10 大阶段，从康德拉季耶夫长波到宏观指标、资产配置、行业轮动和 30 年财富蓝图，
                把“掌握 50 年财富周期”落成可学习、可复盘、可执行的研究路径。
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-10">
              {[
                { value: '65', label: '系统课程' },
                { value: '10', label: '学习阶段' },
                { value: `${Math.round(totalMinutes / 60)}h`, label: '课程时长' },
                { value: '260', label: '章节作业' },
              ].map((item) => (
                ...
                <div key={phase.key} className="mb-14">
                  <div className="flex items-end justify-between gap-4 mb-4">
                    ...
                    <div className="text-xs shrink-0" style={{ color: '#52525b' }}>
                      {phase.courseIds[0]}-{phase.courseIds[phase.courseIds.length - 1]} 课
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {phaseCourses.map((course: any) => {
                      const category = KANGBO_CATEGORIES[course.category] || { label: course.category, icon: '课', color: phase.color };
                          ...
                          <div className="flex items-start gap-3 mb-4">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex flex-wrap gap-1.5 mt-2">
                                ...
                                <span className="text-[11px] px-2 py-0.5" style={{ color: '#71717a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 999 }}>{course.minutes} 分钟</span>
                              </div>
                            </div>
                          </div>

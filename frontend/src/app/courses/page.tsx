      ...
      <div className="relative pt-14 md:pt-20 pb-16 md:pb-24">
        <div className="max-w-5xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 mb-6 text-xs tracking-[0.2em] uppercase"
            ...
            <span className="w-6 h-px" style={{ background: 'rgba(212,168,83,0.15)' }} />
            梦学の殿堂
            <span className="w-6 h-px" style={{ background: 'rgba(212,168,83,0.15)' }} />
          </div>
          ...
          <p className="max-w-xl mx-auto text-sm leading-relaxed mb-3" style={{ color: '#a1a1aa' }}>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 pb-20">
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs tracking-[0.2em] uppercase" style={{ color: 'rgba(212,168,83,0.5)' }}>四大学派</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
          {SCHOOLS.map(school => (
            <div key={school.name} className="p-8" style={{ background: '#111113', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 2 }}>
              {/* 头部 */}
              <div className="flex items-start gap-4 mb-5">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: '#52525b' }}>{school.year}</p>
                </div>
              </div>

              {/* 理论 */}
              <p className="text-sm leading-relaxed mb-5" style={{ color: '#a1a1aa', lineHeight: 1.8 }}>
                {school.theory}
              </p>

              {/* 名言 */}
              <blockquote className="border-l-2 pl-4 mb-5 text-sm italic leading-relaxed"
                style={{ borderColor: school.color + '40', color: '#71717a' }}>
                {school.quote}
              </blockquote>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-8">
            <div style={{ width: 32, height: 1, background: 'rgba(255,255,255,0.06)' }} />
            <span className="text-xs tracking-[0.2em] uppercase" style={{ color: 'rgba(212,168,83,0.5)' }}>学习路线</span>
          </div>
          <h2 className="font-bold mb-2" style={{
            fontFamily: "'Noto Serif SC', serif",
            fontSize: 'clamp(24px, 3vw, 40px)',
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {LEARNING_PATH.map((stage, i) => (
              <div key={stage.stage} className="p-8 text-center relative" style={{ background: '#111113', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 2 }}>
                {/* 阶段号 */}
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full mb-4 mx-auto"
                  style={{ background: '#d4a85315', border: '1px solid #d4a85330' }}>
                  <span className="text-lg font-bold" style={{ color: '#d4a853' }}>{stage.stage}</span>
                </div>
                <div className="text-3xl mb-3">{stage.icon}</div>
                <h3 className="text-lg font-bold mb-2" style={{ fontFamily: "'Noto Serif SC', serif", color: '#f4f4f6' }}>
                  {stage.title}
                </h3>
        ...
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-8">
            <div style={{ width: 32, height: 1, background: 'rgba(255,255,255,0.06)' }} />
            <span className="text-xs tracking-[0.2em] uppercase" style={{ color: 'rgba(212,168,83,0.5)' }}>全部课程</span>
          </div>
        </div>
        ...
        {courses.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p style={{ color: '#71717a' }}>暂无课程数据，请先启动后端服务并运行 seed 脚本</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {courses.map((course: any, idx: number) => {
              const cat = CATEGORIES[course.category] || { label: course.category, icon: '📖', badge: '', color: '#6b5b8a' };
              return (
                <Link
                  key={course.id}
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-lg font-bold mb-1 group-hover:text-[#f4f4f6] transition-colors"
                          style={{ fontFamily: "'Noto Serif SC', serif", color: '#a1a1aa' }}>
                          {course.title}
                        </h3>
